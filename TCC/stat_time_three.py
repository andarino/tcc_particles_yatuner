...
# Tuning process!
tuner.initialize()
tuner.test_run(num_samples=800, warmup=20) #num_samples x 2
tuner.hypotest_optimizers(num_samples=40, num_epochs=60) #num_samples x 2
tuner.hypotest_parameters(num_samples=40) #num_samples x 2
#tuner.optimize(num_samples=2, num_epochs=10) #Using bayesian optimization 
tuner.optimize_linUCB(alpha=0.25,
                      num_bins=30,
                      num_epochs=800, #num_samples x 2
                      nth_choice=4,
                      metric=metric) #using linUCB
tuner.run(num_samples=400) #num_samples x 2
tuner.plot_data() 
